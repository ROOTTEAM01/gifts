<?php
namespace Opencart\Catalog\Model\Extension\Wbcountdown\Module;
class Wbcountdown extends \Opencart\System\Engine\Model {
  ...
  $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
   ...
   return $product_data;
	...
	public function getProductSpecialsCategory($data = array(),$category_id=0) {
	...
	public function getProductsDealCategory($data = array(),$category_id=0) {
		$sql = "SELECT DISTINCT ps.product_id, (
		SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 
		WHERE r1.product_id = ps.product_id AND r1.status = '1' 
		GROUP BY r1.product_id) AS rating 
	...
	public function getSpecialwbcountdown($product_id) {
        if ($this->customer->isLogged()) {
            $customer_group_id = $this->customer->getGroupId();
        } else {
            $customer_group_id = $this->config->get('config_customer_group_id');
