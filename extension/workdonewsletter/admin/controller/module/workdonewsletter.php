<?php   
namespace Opencart\Admin\Controller\Extension\Workdonewsletter\Module;
use \Opencart\System\Helper AS Helper;

class Workdonewsletter extends \Opencart\System\Engine\Controller {

	
	private $error = array();
	private $mdata = array();

	private string $extensionPath = 'extension/workdonewsletter/module/workdonewsletter'; 

	public function initData(){
		$this->load->model('setting/setting');
		$data['workdonewsletter_config'] =1;
        $this->model_setting_setting->editSetting('workdonewsletter_config', $data);
		...
		$this->load->model('extension/workdonewsletter/workdonewsletter/newsletter');
		...
		$this->mdata['action'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter', 'user_token=' . $this->session->data['user_token'], true);
		...
		$menus["dashboard"] = array(
				"link"=>$this->url->link('extension/workdonewsletter/module/workdonewsletter', 'user_token=' . $this->session->data['user_token'], true),
				"title"=>$this->language->get('menu_dashboard')
			);
		
		$menus["subscribes"] = array(
				"link"=>$this->url->link('extension/workdonewsletter/module/workdonewsletter|subsribes', 'user_token=' . $this->session->data['user_token'], true),
				"title"=>$this->language->get('menu_manage_subscribes')
			);

		$menus["modules"] = array(
				"link"=>$this->url->link('extension/workdonewsletter/module/workdonewsletter|modules', 'user_token=' . $this->session->data['user_token'], true),
				"title"=>$this->language->get('menu_manage_modules')	
			);

		$menus["draft_contact"] = array(
				"link"=>$this->url->link('extension/workdonewsletter/module/workdonewsletter|draft_contact', 'user_token=' . $this->session->data['user_token'], true),
				"title"=>$this->language->get('menu_draft_contact')
			);

		$menus["contact"] = array(
				"link"=>$this->url->link('extension/workdonewsletter/module/workdonewsletter|contact', 'user_token=' . $this->session->data['user_token'], true),
				"title"=>$this->language->get('menu_contact')
			);

   		$this->mdata["menus"] = $menus;
		
   		$this->document->addStyle('view/stylesheet/workdonewsletter.css');
	}
	...
	public function _getStores(){
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|modules', $url.'&user_token=' . $this->session->data['user_token'], true);
		}
		return $stores;
	}
	...
	public function setBreadcrumb(){
		...
		$this->mdata['breadcrumbs'] = array();
   		...
   		$this->mdata['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/workdonewsletter/module/workdonewsletter', 'user_token=' . $this->session->data['user_token'], true),
      		'separator' => ' :: '
   		);
	}
	public function index() {
		...
		$model = $this->model_extension_workdonewsletter_workdonewsletter_newsletter;

		$model->installModule();

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('workdonewsletter', $this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter', 'user_token=' . $this->session->data['user_token'], true));
		}

		if( isset($this->request->get['module_id']) && $this->request->get['module_id'] > 0 ){
			$this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter|modules', 'user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id'], true));
		}
		$this->setBreadcrumb();

		// Get Data Setting
		$this->mdata['modules'] = array();
		if (isset($this->request->post['workdonewsletter_module'])) {
			$this->mdata['modules'] = $this->request->post['workdonewsletter_module'];
		} elseif ($this->config->get('workdonewsletter_module')) {
			$this->mdata['modules'] = $this->config->get('workdonewsletter_module');
		}

		$this->mdata['general'] = $this->config->get('workdonewsletter_config');
		...
		$template = 'extension/workdonewsletter/module/workdonewsletter/panel';
		$this->response->setOutput($this->load->view($template, $this->mdata));
	
	}
	public function subsribes(){
		$this->initData();
		$this->load->model('extension/workdonewsletter/workdonewsletter/subscribe');
		$this->load->model('setting/store');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if(isset($post) && $post['action'] == "delete" && !empty($post['selected'])){
				...
				foreach ($selected as $key => $value) {
					$this->model_extension_workdonewsletter_workdonewsletter_subscribe->delete($value);
				}
			...
			$this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter|subsribes', 'user_token=' . $this->session->data['user_token'], true));
		}
		...
		$this->mdata['action'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|subsribes', 'user_token=' . $this->session->data['user_token'] . $url, true);

		$this->mdata["menu_active"] = "subscribes";

		$subscribe_total = $this->model_extension_workdonewsletter_workdonewsletter_subscribe->getTotalSubscribers($data);
		$results 		 = $this->model_extension_workdonewsletter_workdonewsletter_subscribe->getSubscribers($data);
		$stores  		 = $this->model_setting_store->getStores();
		...
		$customer_groups = $this->model_extension_workdonewsletter_workdonewsletter_subscribe->getCustomerGroups();
		$tmp = array();
		...
		foreach ($results as $result) {
			...
			if($result['action'] == 1){
				...
				'href' => $this->url->link('extension/workdonewsletter/module/workdonewsletter|unsubsribe', 'user_token=' . $this->session->data['user_token'] . '&subscribe_id=' . $result['subscribe_id'] . $url, true)
				);
			}else{
				$action_name =  $this->language->get('text_no');
				$action[] = array(
				'text' => $this->language->get('text_subscribe'),
				'href' => $this->url->link('extension/workdonewsletter/module/workdonewsletter|subsribe', 'user_token=' . $this->session->data['user_token'] . '&subscribe_id=' . $result['subscribe_id'] . $url, true)
				);
			}
		...
		$this->mdata['sort_name'] 			   = $this->url->link('extension/module/workdonewsletter/subsribes', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
		$this->mdata['sort_email'] 			   = $this->url->link('extension/module/workdonewsletter/subsribes', 'user_token=' . $this->session->data['user_token'] . '&sort=email' . $url, true);
		$this->mdata['sort_subscribe'] 		   = $this->url->link('extension/module/workdonewsletter/subsribes', 'user_token=' . $this->session->data['user_token'] . '&sort=s.action' . $url, true);
		$this->mdata['sort_customer_group_id'] = $this->url->link('extension/module/workdonewsletter/subsribes', 'user_token=' . $this->session->data['user_token'] . '&sort=customer_group_id' . $url, true);
		$this->mdata['sort_store_id'] 		   = $this->url->link('extension/module/workdonewsletter/subsribes', 'user_token=' . $this->session->data['user_token'] . '&sort=s.store_id' . $url, true);
		...
		$this->mdata['pagination'] = $this->load->controller('common/pagination', [
			'total' => $subscribe_total,
			'page'  => $data['page'],
			'limit' => $this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('extension/workdonewsletter/module/workdonewsletter|subsribes', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}')
		]);
		$this->mdata['results'] = sprintf($this->language->get('text_pagination'), ($subscribe_total) ? (($data['page'] - 1) * $this->config->get('config_pagination_admin')) + 1 : 0, ((($data['page'] - 1) * $this->config->get('config_pagination_admin')) > ($subscribe_total - $this->config->get('config_pagination_admin'))) ? $subscribe_total : ((($data['page'] - 1) * $this->config->get('config_pagination_admin')) + $this->config->get('config_pagination_admin')), $subscribe_total, ceil($subscribe_total / $this->config->get('config_pagination_admin')));
		...
		$pagination->url   = $this->url->link('extension/opencart/module/workdonewsletter|subsribes', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
*/
		...
		$template = 'extension/workdonewsletter/module/workdonewsletter/subscribes';
	...
	public function unsubsribe(){
		...
		if(!empty($subscribe_id)){
			$this->model_extension_workdonewsletter_workdonewsletter_subscribe->updateAction($subscribe_id, 0);
		}
		$this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter|subsribes', 'user_token=' . $this->session->data['user_token'], true));
	}
	...
	public function subsribe(){
		...
		if(!empty($subscribe_id)){
			$this->model_extension_workdonewsletter_workdonewsletter_subscribe->updateAction($subscribe_id, 1);
		}
		$this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter|subsribes', 'user_token=' . $this->session->data['user_token'], true));
	}
	public function draft(){
		$this->initData();
		$this->setBreadcrumb();
		$this->load->model("extension/workdonewsletter/workdonewsletter/template");
    	$this->load->model("extension/workdonewsletter/workdonewsletter/draft");
		$this->mdata["menu_active"] = "draft";
		$this->mdata['cancel'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|templates', 'user_token=' . $this->session->data['user_token'], true);
		$template_id = isset($this->request->get['id'])?$this->request->get['id']:0;
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			  $action = isset($this->request->post["action"])?$this->request->post["action"]:"";
        if($action == "delete"){
            foreach ($this->request->post['selected'] as $draft_id) {
                $this->model_extension_workdonewsletter_workdonewsletter_draft->delete($draft_id);
            }
            $this->session->data['success'] = $this->language->get('text_success');
        }

        $this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter|draft', 'user_token=' . $this->session->data['user_token'], true));
      ...
      $total = $this->model_extension_workdonewsletter_workdonewsletter_draft->getTotal($data);

      $this->mdata['draft'] = array();

      $results = $this->model_extension_workdonewsletter_workdonewsletter_draft->getList($data);
      ...
      $this->mdata['detail'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|create_newsletter', 'user_token=' . $this->session->data['user_token'] . $url . '&id=', true);
      $this->mdata['action'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|draft', 'user_token=' . $this->session->data['user_token'], true);
      ...
      $pagination->limit = $this->config->get('config_admin_limit');
      $pagination->text  = $this->language->get('text_pagination');
      $pagination->url   = $this->url->link('extension/workdonewsletter/module/workdonewsletter|draft', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
      ...
      $template = 'extension/workdonewsletter/module/workdonewsletter/draft_newsletter';
	...
	public function get_template(){
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (isset($this->request->post['store_id'])) {
				...
				if ($store) {
					$url = rtrim($store['url'], '/') . '/index.php?route=extension/workdonewsletter/module/workdonewsletter/get_template/json';
				} else {
					$url = $store_url . 'index.php?route=extension/workdonewsletter/module/workdonewsletter/get_template/json';
				}
			} else {
				$url = $store_url . 'index.php?route=extension/workdonewsletter/module/workdonewsletter/get_template/json';
			}
			...
			$this->response->redirect($this->url->link('extension/module/workdonewsletter/create_newsletter', 'user_token=' . $this->session->data['user_token'], true));
		}
	}
	private function do_request($options) {
		...
		curl_setopt($ch, CURLOPT_USERAGENT, 'workdonewsletter for Opencart');
	...
	public function save_draft(){
      if ($this->request->server['REQUEST_METHOD'] == 'POST') {
          $this->load->model('extension/workdonewsletter/workdonewsletter/draft');
          $this->load->language('extension/workdonewsletter/module/workdonewsletter');
          $this->model_extension_workdonewsletter_workdonewsletter_draft->save($this->request->post);
	...
	/*	if (!$this->user->hasPermission('modify', 'extension/opencart/module/workdonewsletter')) {
	...
	public function draft_contact(){
		...
		$this->mdata['url_delete'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|draft_contact', 'user_token=' . $this->session->data['user_token'], true);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			if ( isset($this->request->post['selected']) ) {
				foreach ($this->request->post['selected'] as $draft_id) {
					$this->model_extension_workdonewsletter_workdonewsletter_draft->deleteDraft($draft_id);
				}
				$this->session->data['success'] = $this->language->get('text_success');
				$this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter|draft_contact', 'user_token=' . $this->session->data['user_token'], true));
			}
		}
		...
		$draft_total = $this->model_extension_workdonewsletter_workdonewsletter_draft->getTotalDraft();

		$results = $this->model_extension_workdonewsletter_workdonewsletter_draft->getDrafts($filter_data);
		...
		foreach ($results as $result) {
			$this->mdata['drafts'][] = array(
				...
				'edit'       => $this->url->link('extension/workdonewsletter/module/workdonewsletter|contact', 'user_token=' . $this->session->data['user_token'] . '&draft_id=' . $result['draft_id'] . $url, true),
				'delete'     => $this->url->link('extension/workdonewsletter/module/workdonewsletter|draft_contact/delete', 'user_token=' . $this->session->data['user_token'] . '&draft_id=' . $result['draft_id'] . $url, true)
			);
		}
		...
		$this->mdata['sort_subject'] = $this->url->link('extension/module/workdonewsletter/draft_contact', 'user_token=' . $this->session->data['user_token'] . '&sort=subject' . $url, true);
		$this->mdata['sort_to'] = $this->url->link('extension/module/workdonewsletter/draft_contact', 'user_token=' . $this->session->data['user_token'] . '&sort=to' . $url, true);
		...
		$this->mdata['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('extension/workdonewsletter/module/workdonewsletter|draft_contact', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}')
		]);

		$this->mdata['results'] = sprintf($this->language->get('text_pagination'), ($draft_total) ? (($page - 1) * $this->config->get('config_pagination_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_pagination_admin')) > ($draft_total - $this->config->get('config_pagination_admin'))) ? $draft_total : ((($page - 1) * $this->config->get('config_pagination_admin')) + $this->config->get('config_pagination_admin')), $draft_total, ceil($draft_total / $this->config->get('config_pagination_admin')));

		// PAGINAtiON 
/*		$pagination = new Pagination();
		$pagination->total = $draft_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_pagination_admin');
		$pagination->url = $this->url->link('extension/opencart/module/workdonewsletter|draft_contact', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		$this->mdata['pagination'] = $pagination->render();*/
		...
		$this->mdata['add'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|contact', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$this->mdata['repair'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|draft_contact', 'user_token=' . $this->session->data['user_token'] . $url, true);

		// LOAD TEMPLATE
		$template = 'extension/workdonewsletter/module/workdonewsletter/draft_contact';
		$this->_render($template);
	}
	...
	public function contact(){
		...
		$this->mdata['url_draft'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|ajaxdraft', 'user_token=' . $this->session->data['user_token'], true);
		$this->mdata['url_send'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|ajaxsend', 'user_token=' . $this->session->data['user_token'], true);
		...
		if (isset($this->request->get['draft_id'])) {
			$draft_info = $this->model_extension_workdonewsletter_workdonewsletter_draft->getDraft($this->request->get['draft_id']);
		...
		// LOAD TEMPLATE
		$template = 'extension/workdonewsletter/module/workdonewsletter/contact';
	...
	public function ajaxdraft(){
		...
		/*	if (!$this->user->hasPermission('modify', 'extension/opencart/module/workdonewsletter')) {
			...
			if (!$json) {
				...
				// SAVE DATA
				$this->load->model('extension/workdonewsletter/draft');

				$this->model_extension_workdonewsletter_workdonewsletter_draft->savebasic($this->request->post);
	...
	public function ajaxsend() {
		$this->load->language('extension/module/workdonewsletter');
			...
			if (!$json) {
				...
				switch ($this->request->post['to']) {
					case 'list_subscribes':
						$email_total = $this->model_extension_workdonewsletter_workdonewsletter_draft->getTotalEmail();
						$results = $this->model_extension_workdonewsletter_workdonewsletter_draft->getEmails();
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('workdonewsletter/contact/send', 'user_token=' . $this->session->data['user_token'] . '&page=' . ($page + 1), true));
					} else {
						$json['next'] = '';
					}
	...
	public function create_newsletter(){
		...
		if( isset($this->request->get['id']) ){
			$this->request->post = $this->model_extension_workdonewsletter_workdonewsletter_draft->detail($this->request->get['id']);
		}
		...
		$this->initData();
		$this->setBreadcrumb();
		$this->load->model("workdonewsletter/template");
		$this->load->model("workdonewsletter/newsletter");
		$this->mdata["menu_active"] = "create_newsletter";
		$this->mdata['cancel'] = $this->url->link('extension/module/workdonewsletter/templates', 'user_token=' . $this->session->data['user_token'], true);
		$this->mdata["templates"] = $this->model_extension_workdonewsletter_workdonewsletter_template->getTemplates();
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateSend()) { 
			...
			elseif ($action =='check_spam') {
				...
				switch ($this->request->post['to']) {
					case 'subscriber':
						...
						$results = $this->model_extension_workdonewsletter_workdonewsletter_newsletter->getCustomers($customer_data);
					...
					case 'all':
						...
						$results = $this->model_extension_workdonewslettert_workdonewsletter_newsletter->getCustomers();
						...
						break;
					case 'newsletter':
							$this->load->model('extension/workdonewsletter/subscribe');
							$customer_data = array(
								'filter_action' => 1
							);

							$results = $this->model_extension_workdonewsletter_workdonewsletter_subscribe->getSubscribers($customer_data); 
						...
						break;
					case 'customer_all':
						$results = $this->model_extension_workdonewsletter_workdonewsletter_newsletter->getCustomers();
					...
					case 'customer':
						if (isset($this->request->post['customer']) && !empty($this->request->post['customer'])) {
							foreach ($this->request->post['customer'] as $customer_id) {
								$customer_info = $this->model_extension_workdonewsletter_workdonewsletter_newsletter->getCustomer($customer_id);
						...
						break;
					case 'affiliate_all':
						$results = $this->model_extension_workdonewsletter_workdonewsletter_newsletter->getAffiliates();
					...
					case 'affiliate':
						if (isset($this->request->post['affiliate']) && !empty($this->request->post['affiliate'])) {
							foreach ($this->request->post['affiliate'] as $affiliate_id) {
								$affiliate_info = $this->model_extension_workdonewsletter_workdonewsletter_newsletter->getAffiliate($affiliate_id);
					...
					case 'product':
						if (isset($this->request->post['product']) && $this->request->post['product']) {
							$results = $this->model_extension_workdonewsletter_workdonewsletter_newsletter->getEmailsByProductsOrdered($this->request->post['product']);
						...
						break;
					case 'rewards_all':
						$results = $this->model_extension_workdonewsletter_workdonewsletter_newsletter->getRecipientsWithRewardPoints();
						...
						break;
					case 'rewards':
						$results = $this->model_extension_workdonewsletter_workdonewsletter_newsletter->getSubscribedRecipientsWithRewardPoints();
			...
			if ($emails) {
				...
				$data_products = array();
				//get list products
				$this->load->model('extension/workdonewsletter/workdonewsletter/product');
				$setting = array(
				...
				if($this->request->post['defined'] && isset($this->request->post['defined_product']) ) {
					...
					$products = $this->model_extension_workdonewsletter_workdonewsletter_product->getProducts();
				...
				if($this->request->post['special']) {
					$specialProducts = $this->model_extension_workdonewsletter_workdonewsletter_product->getProductSpecials($setting['limit']);
					$data_products['special'] = $this->getItemProducts($specialProducts, $setting);
				}
				
				if($this->request->post['latest']) {
					$latestProducts = $this->model_extension_workdonewsletter_workdonewsletter_product->getLatestProducts($setting['limit']);
					$data_products['latest'] = $this->getItemProducts($latestProducts, $setting);
				}
				
				if($this->request->post['popular']) {
					$popularProducts = $this->model_extension_workdonewsletter_workdonewsletter_product->getPopularProducts($setting['limit']);
				...
				$this->model_extension_workdonewsletter_workdonewsletter_newsletter->send($data);
			} else {
		...
		if (isset($this->error['message'])) {
  		...
  		$this->mdata['breadcrumbs'] = array();
   		...
   		$this->mdata['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('extension/workdonewsletter/module/workdonewsletter|create_newsletter', 'user_token=' . $this->session->data['user_token'], true),
      		'separator' => ' :: '
   		);
		...
		$this->mdata['action'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|create_newsletter', 'user_token=' . $this->session->data['user_token'], true);

    	$this->mdata['save'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|save_draft', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->load->model('extension/workdonewsletter/workdonewsletter/template');
		$templates = $this->model_extension_workdonewsletter_workdonewsletter_template->getTemplates();
		...
		foreach ($templates as $template) {
			$this->mdata['templates'][] = array(
					...
					'link'          => $this->url->link('extension/module/workdonewsletter/template', '&id='.$template['template_id'].'&user_token='.$this->session->data['user_token'], true)
				);
		}
    ...
    $this->mdata['affiliates'] = array();
      ...
      if (isset($this->request->post['affiliate']) && is_array($this->request->post['affiliate'])) {
		...
		$template = 'extension/workdonewsletter/module/workdonewsletter/form_newsletter';
		$this->_render($template);
	}
	...
	public function templates(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			switch ($action) {
				...
				case 'copy':
					...
					$check = false;
					if(!empty($templates)){
						$check = $this->model_extension_workdonewsletter_workdonewsletter_template->copy($templates);
					}
					if($check){
				...
				case 'delete':
					...
					$check = false;
					if(!empty($templates)){
						$check = $this->model_extension_workdonewsletter_workdonewsletter_template->delete($templates);
					}
					if($check){
		...
		foreach ($templates as $template) {
			$this->mdata['templates'][] = array(
					...
					'date_added'    => $template['date_added'],
					'date_modified' => $template['date_modified'],
					'link'          => $this->url->link('extension/workdonewsletter/module/workdonewsletter|template', '&id='.$template['template_id'].'&user_token='.$this->session->data['user_token'], true)
				);
		}
		...
		$this->mdata['insert_link'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|template', 'user_token=' . $this->session->data['user_token'], true);
		$this->mdata['action']      = $this->url->link('extension/workdonewsletter/module/workdonewsletter|templates', 'user_token=' . $this->session->data['user_token'], true);
		
		$template = 'extension/workdonewsletter/module/workdonewsletter/templates';
		$this->_render($template);
	}
	...
	/*	if (!$this->user->hasPermission('modify', 'extension/workdonewsletter/module/workdonewsletter')) {
		...
		if (!isset($json['error'])) {
			if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name'])) {
				...
				move_uploaded_file($this->request->files['file']['tmp_name'], DIR_DOWNLOAD .'workdonewsletter/'. $filename . '.' . $ext);
			}
	...
	public function template_from_file(){
		...
		if(!empty($template_file)){
			$file_path = DIR_DOWNLOAD."workdonewsletter/".$template_file;
	...
	public function template(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if($action == "get_template"){
				...
				$check = $this->model_extension_workdonewsletter_workdonewsletter_template->insertTemplate($this->request->post);
				if($check)
				 	$this->session->data['success'] = $this->language->get('text_workdonewsletter_success');
				else
					$this->session->data['error_warning'] = $this->language->get('text_workdonewsletter_error_warning');

				$this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter|templates', 'user_token=' . $this->session->data['user_token'], true));
			}

		}
		$template = $this->model_extension_workdonewsletter_workdonewsletter_template->getTemplate($template_id);
		$this->load->model('localisation/language');
		$languages = $this->model_localisation_language->getLanguages();
		$this->mdata['languages'] = $languages;
		...
		$this->mdata['action'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|template', 'user_token=' . $this->session->data['user_token'], true);
		

		$template = 'extension/workdonewsletter/module/workdonewsletter|form_template';
		$this->_render($template);
	}
	public function modules(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('workdonewsletter.workdonewsletter', $this->request->post);
				$this->response->redirect( $this->url->link('extension/workdonewsletter/module/workdonewsletter|modules', 'user_token=' . $this->session->data['user_token'], true) );
			} else {
		...
		$this->mdata['extensions'] = $this->module("workdonewsletter", "&module_id");
		...
		// action
		$this->mdata['delete'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|ndelete', 'user_token=' . $this->session->data['user_token'].$url, true);
		$this->mdata['action'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|modules', 'user_token=' . $this->session->data['user_token'].$url, true);
		...
		$template = 'extension/workdonewsletter/module/workdonewsletter/frontend_modules';
		$this->_render($template);
	}
	...
	public function module($extension, $module_id){
		...
		foreach ($modules as $module) {
			$module_data[] = array(
				...
				'name'      => $module['name'],
				'edit'      => $this->url->link('extension/workdonewsletter/module/workdonewsletter|modules', 'user_token=' . $this->session->data['user_token'] . $module_id.'=' . $module['module_id'], true),
			);
		}
		$ex[] = array(
			'name'      => $this->language->get("create_module"),
			'module'    => $module_data,
			'edit'      => $this->url->link('extension/workdonewsletter/module/workdonewsletter|modules', 'user_token=' . $this->session->data['user_token'], true)
		);
		return $ex;
	}
	...
	public function config(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('workdonewsletter_config', $this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->response->redirect($this->url->link('extension/workdonewsletter/module/workdonewsletter|config', 'user_token=' . $this->session->data['user_token'], true));
		}
    	...
    	$this->mdata['action'] = $this->url->link('extension/workdonewsletter/module/workdonewsletter|config', 'user_token=' . $this->session->data['user_token'], true);

    	// Get Data Setting
		$this->mdata['general'] = array();
		if (isset($this->request->post['workdonewsletter_config'])) {
			$this->mdata['general'] = $this->request->post['workdonewsletter_config'];
		} elseif ($this->config->get('workdonewsletter_config')) {
			$this->mdata['general'] = $this->config->get('workdonewsletter_config');
		}

		// Render
		$template = 'extension/workdonewsletter/module/workdonewsletter|config';
		$this->_render($template);
	}
	...
	public function getItemProducts($results , $setting){
