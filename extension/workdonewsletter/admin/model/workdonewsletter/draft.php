...
namespace Opencart\Admin\Model\Extension\Workdonewsletter\Workdonewsletter;
	...
	class Draft extends \Opencart\System\Engine\Model {

	// inspireo 2.2 edited
	public function getTotalEmail() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "workdonewsletter_subscribe");
		return $query->row['total'];
	}

	public function getEmails() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "workdonewsletter_subscribe");
		return $query->rows;
	}

	public function deletedraft($draft_id){
		$this->db->query("DELETE FROM " . DB_PREFIX . "workdonewsletter_draft WHERE draft_id = '" . (int)$draft_id . "'");
	}

	public function getTotalDraft() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "workdonewsletter_draft");
		return $query->row['total'];
	}

	public function getDraft($draft_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "workdonewsletter_draft WHERE draft_id = " . (int)$draft_id);
		return $query->row;
	}

	public function getDrafts($data = array()) {

		$sql = "SELECT * FROM " . DB_PREFIX . "workdonewsletter_draft";
	...
	public function savebasic($data){
		...
		if (isset($data['draft_id']) && $data['draft_id'] != 0) {
			$this->db->query("UPDATE " . DB_PREFIX . "workdonewsletter_draft SET 
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "workdonewsletter_draft SET 
				`date_added` = NOW(),
				`to` = '" . $this->db->escape($data['to']) . "',
				`store_id` = '" . (int)$data['store_id'] . "',
				`subject` = '" . $this->db->escape($data['subject']) . "',
	...
	public function getTotal($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "workdonewsletter_email` WHERE 1=1";
	...
	public function getList($data = array()) {
		if ($data) {
			$sql = "SELECT `email_id`, `subject`, `date_added`, `to`, `store_id` FROM " . DB_PREFIX . "workdonewsletter_email WHERE 1=1"; 
	...
	public function detail($email_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "workdonewsletter_email WHERE email_id = '" . (int)$email_id . "'"); 
		$data = $query->row;
	...
	public function delete($email_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "workdonewsletter_email WHERE email_id = '" . (int)$email_id . "'");
