...
class Language extends \Opencart\System\Engine\Model {
	public function addLanguage(array $data): int {
		...
		foreach ($query->rows as $banner_image) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "banner_image` SET `banner_id` = '" . (int)$banner_image['banner_id'] . "', `language_id` = '" . (int)$language_id . "', `title` = '" . $this->db->escape($banner_image['title']) . "', `link` = '" . $this->db->escape($banner_image['link']) . "', `description` = '" . $this->db->escape($banner_image['description']) . "',  `image` = '" . $this->db->escape($banner_image['image']) . "', `sort_order` = '" . (int)$banner_image['sort_order'] . "'");
		}
